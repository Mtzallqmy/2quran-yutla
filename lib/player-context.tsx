...
type PlayerValue = { current: AudioItem | null; isPlaying: boolean; error: string | null; favorites: string[]; playlists: Playlist[]; recentIds: string[]; playItem: (item: AudioItem) => Promise<void>; togglePlayback: () => void; stopPlayback: () => void; reconnect: () => Promise<void>; toggleFavorite: (id: string) => void; createPlaylist: (title: string) => void; addToPlaylist: (playlistId: string, itemId: string) => void; };
...
export function PlayerProvider({ children }: PropsWithChildren) {
  ...
  const [current, setCurrent] = useState<AudioItem | null>(null); const [isPlaying, setIsPlaying] = useState(false); const [error, setError] = useState<string | null>(null);
  ...
  const playItem = useCallback(async (item: AudioItem) => {
    const token = ++requestId.current; setError(null); releaseCurrent();
    try {
      await setAudioModeAsync({ playsInSilentMode: true });
      if (token !== requestId.current) return;
      const next = createAudioPlayer(item.streamUrl);
      if (token !== requestId.current) { next.remove(); return; }
      audio.current = next; next.play(); setCurrent(item); setIsPlaying(true);
  ...
  const stopPlayback = useCallback(() => { ++requestId.current; releaseCurrent(); setCurrent(null); haptic.light(); }, [releaseCurrent]);
  ...
  const reconnect = useCallback(async () => { if (current) await playItem(current); }, [current, playItem]);
  ...
  const value = useMemo<PlayerValue>(() => ({ current, isPlaying, error, favorites, playlists, recentIds, playItem, togglePlayback, stopPlayback, reconnect, toggleFavorite, createPlaylist, addToPlaylist }), [addToPlaylist, createPlaylist, current, error, favorites, isPlaying, playlists, playItem, recentIds, reconnect, stopPlayback, toggleFavorite, togglePlayback]);
